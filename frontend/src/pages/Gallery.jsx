...
const TILES = [
  { img: 'heroVelvet', cat: 'flocked', label: 'Flocked — Navy pile, loom-state', aspect: 'aspect-[3/4]' },
  { img: 'jewelBox', cat: 'flocked', label: 'Flocked — Non-woven box lining', aspect: 'aspect-square' },
  { img: 'giftPink', cat: 'flocked', label: 'Flocked — Gift & décor finish', aspect: 'aspect-[4/3]' },
  { img: 'sneaker', cat: 'flocked', label: 'Flocked — PVC base for footwear', aspect: 'aspect-[4/3]' },
  { img: 'sofaGreen', cat: 'weaving', label: 'Weaving — Holland on the frame', aspect: 'aspect-[4/3]' },
  { img: 'sofaGrey', cat: 'weaving', label: 'Weaving — FD full-dull lustre', aspect: 'aspect-[3/4]' },
  { img: 'bedroomLuxe', cat: 'weaving', label: 'Weaving — Kabul, heavy body', aspect: 'aspect-[3/4]' },
  { img: 'chairYellow', cat: 'weaving', label: 'Weaving — Raised pile detail', aspect: 'aspect-square' },
  { img: 'blackTexture', cat: 'knitting', label: 'Knitting — Micro 11000 “Falcon”', aspect: 'aspect-square' },
  { img: 'dressFlow', cat: 'knitting', label: 'Knitting — Viscose drape', aspect: 'aspect-[3/4]' },
  { img: 'fabricRack', cat: 'knitting', label: 'Knitting — Micro 9000 face', aspect: 'aspect-[4/3]' },
  { img: 'saree', cat: 'flocked', label: 'Flocked — Gota design, festive', aspect: 'aspect-[3/4]' },
...
const Gallery = () => {
  ...
  return (
    <div data-testid="gallery-page">
      <PageHero
        id="gallery-hero"
        overline="Texture Archive — Placeholder Photography"
        lines={[<>Seen up close,</>, <><em className="italic text-navy">felt in bulk.</em></>]}
        right={
          <p>
            A working archive of textures by construction. Final mill photography replaces
            these frames as each lot is shot — the grid stays, the images get real.
          </p>
        }
      />
      ...
      <section className="px-6 py-12 md:px-12 md:py-16">
            ...
            <Reveal key={`${filter}-${i}`} delay={(i % 3) * 0.07} className="mb-6 break-inside-avoid">
              <figure className="group" data-testid={`gallery-tile-${t.cat}-${i}`}>
                <div className={`img-frame ${t.aspect} border border-navy/20`}>
                  <img src={IMAGES[t.img]} alt={t.label} loading="lazy" className="transition-transform duration-700" />
        ...
        <p className="mx-auto mt-6 max-w-[1600px] font-mono text-[10px] uppercase tracking-[0.22em] text-navy/50" data-testid="gallery-note">
          Photography shown is representative placeholder — mill-shot imagery follows per lot.
        </p>
      </section>
    </div>
  );
